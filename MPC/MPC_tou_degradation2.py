...
def predict_day(start):
    ...
    for index in range(24):
        ...
        if index==0:
            ...
            twoac = clf_ac.predict([[day_state[index][1], day_state[index][2], day_state[index][3], day_state[index][4], day_state[index][6], day_state[index-1][11], day_state[index][13], day_state[index][7], day_state[index][8]]])[0]
            ac = twoac*2
            #update
...
def compute(hour_var,battery_var,last):
    ...
    for t in range(MAX_TS):
        ...
        constraints += [U[t] == E[t+1]-E[t]]

    # NEM with a different export tariff than ToU
    objective = cvx.Minimize(cvx.sum([FG[t]*P_grid[t]+TG[t]*P_solar[t]+4.5e-4*(cvx.atoms.elementwise.abs.abs(U[t])**1.3)*6500/2 for t in range(MAX_TS)]))
    ...
    #cycles=rainflow.count_cycles(E)
    #print("this is cycles-----------------------------------------")
    # for low, high, mult in rainflow.extract_cycles(E):
    #     print(low,high,mult)
...
if __name__ == '__main__':
    ...
    pathlib.Path("mpcnew_4/{}_4_mpcnew".format(homeid)).mkdir(parents=True, exist_ok=True)
    csvfile = open("mpcnew_4/{}_4_mpcnew/sb".format(homeid)+str(int(float(sys.argv[1])*100))+"b"+str(int(float(sys.argv[2])*10))+".csv", 'w', newline='')
