...
import os
import pathlib
from environment_bill_rbc import EnergyEnvironment
...
actionlist=pd.read_csv("rbc_4/{}_4_rbc/sb".format(homeid)+str(int(float(sys.argv[1])*100))+"b"+str(int(float(sys.argv[2])*10))+".csv")['Base_Action']
...
pathlib.Path("rbc_4_bill/{}_4_rbc_bill".format(homeid)).mkdir(parents=True, exist_ok=True)
csvfile = open("rbc_4_bill/{}_4_rbc_bill/sb".format(homeid)+str(int(float(sys.argv[1])*100))+"b"+str(int(float(sys.argv[2])*10))+".csv", 'w', newline='')
