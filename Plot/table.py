...
csvfile = open("violin/violin_new.csv", 'w', newline='')
writer = csv.writer(csvfile, delimiter=',')
writer.writerow(["Home","Bill","Panel number","Battery size","Method"])
...
for i in hhids:
    ...
    writer.writerow([bill,0,0,"RBC"])
    writer.writerow([bill,0,6.4,"RBC"])
    writer.writerow([bill,0,13.5,"RBC"])
    writer.writerow([bill,0,0,"RL"])
    writer.writerow([bill,0,6.4,"RL"])
    writer.writerow([bill,0,13.5,"RL"])
    writer.writerow([bill,0,0,"MPC"])
    writer.writerow([bill,0,6.4,"MPC"])
    writer.writerow([bill,0,13.5,"MPC"])
    writer.writerow([bill,0,0,"Optimal"])
    writer.writerow([bill,0,6.4,"Optimal"])
    writer.writerow([bill,0,13.5,"Optimal"])

    table = pd.read_csv('nostorage_2/{}_2_nostorage/sb4b0.csv'.format(i))
    bill = table['Base_Bill'].iloc[8735]-table['Base_Bill'].iloc[167]
    writer.writerow([bill,2,0,"RBC"])
    writer.writerow([bill,2,0,"RL"])
    writer.writerow([bill,2,0,"Optimal"])
    writer.writerow([bill,2,0,"MPC"])

    table = pd.read_csv('rbc_2/{}_2_rbc/sb4b64.csv'.format(i))
    bill = table['Base_Bill'].iloc[8735]-table['Base_Bill'].iloc[167]
    writer.writerow([bill,2,6.4,"RBC"])
    table = pd.read_csv('rbc_2/{}_2_rbc/sb4b135.csv'.format(i))
    bill = table['Base_Bill'].iloc[8735]-table['Base_Bill'].iloc[167]
    writer.writerow([bill,2,13.5,"RBC"])

    table = pd.read_csv('mpc_2_bill/{}_2_mpc_bill/sb4b64.csv'.format(i))
    bill = table['Bill'].iloc[-1]
    writer.writerow([bill,2,6.4,"MPC"])
    table = pd.read_csv('mpc_2_bill/{}_2_mpc_bill/sb4b135.csv'.format(i))
    bill = table['Bill'].iloc[-1]
    writer.writerow([bill,2,13.5,"MPC"])

    table = pd.read_csv('rl_2_bill/{}_2_pre2_bill/sb4b64.csv'.format(i))
    bill = table['Bill'].iloc[-1]
    writer.writerow([bill,2,6.4,"RL"])
    table = pd.read_csv('rl_2_bill/{}_2_pre2_bill/sb4b135.csv'.format(i))
    bill = table['Bill'].iloc[-1]
    writer.writerow([bill,2,13.5,"RL"])

    table = pd.read_csv('op_2/{}_2_op/sb-4b64.csv'.format(i))
    bill = table['total_reward'].iloc[8735]-table['total_reward'].iloc[167]
    writer.writerow([bill,2,6.4,"Optimal"])
    table = pd.read_csv('op_2/{}_2_op/sb-4b135.csv'.format(i))
    bill = table['total_reward'].iloc[8735]-table['total_reward'].iloc[167]
    writer.writerow([bill,2,13.5,"Optimal"])
...
# bills = pd.read_csv("violin/violin_new.csv")
...
# ax = sns.catplot(x="Method", y="Bill", kind="swarm", data=bills,palette="muted");
#
#
# plt.savefig("violin/4-3.png")
# plt.show()
