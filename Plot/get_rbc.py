...
def pre_train(episodes):
    ...
    for ep in range(episodes):
        ...
        for t in range(MAX_EP_STEPS):
            ...
            ACTION_BOUND = [-min( s[8], s[5], MAX_CHARGE_RATE), min((env.maximum_battery - s[8]), MAX_CHARGE_RATE)]
...
if __name__ == '__main__':
    if LOAD:
        ...
        pathlib.Path("rbc_2_test/{}_2_rbc".format(homeid)).mkdir(parents=True, exist_ok=True)
        csvfile = open("rbc_2_test/{}_2_rbc/sb".format(homeid)+str(int(float(sys.argv[1])*100))+"b"+str(int(float(sys.argv[2])*10))+".csv", 'w', newline='')
