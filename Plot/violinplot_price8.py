...
csvfile = open("violin/violin_policy8.csv", 'w', newline='')
...
for i in hhids:
    ...
    table = pd.read_csv('rbc_2/{}_2_rbc/sb8b64.csv'.format(i))
    bill = table['Base_Bill'].iloc[8735]-table['Base_Bill'].iloc[167]
    writer.writerow([bill/1.3,2,6.4,"RBC"])
    table = pd.read_csv('rbc_2/{}_2_rbc/sb8b135.csv'.format(i))
    bill = table['Base_Bill'].iloc[8735]-table['Base_Bill'].iloc[167]
    #writer.writerow([bill/1.3,2,13.5,"RBC"])

    table = pd.read_csv('rl_2_bill/{}_2_pre2_bill/sb8b64.csv'.format(i))
    bill = table['Bill'].iloc[-1]
    writer.writerow([bill/1.3,2,6.4,"RL"])
    table = pd.read_csv('rl_2_bill/{}_2_pre2_bill/sb8b135.csv'.format(i))
    bill = table['Bill'].iloc[-1]
    #writer.writerow([bill/1.3,2,13.5,"RL"])


    table = pd.read_csv('olc_2_bill/{}_2_olc_bill/sb8b64.csv'.format(i))
    bill = table['Bill'].iloc[-1]
    writer.writerow([bill/1.3,2,6.4,"DLC"])
    table = pd.read_csv('olc_2_bill/{}_2_olc_bill/sb8b135.csv'.format(i))
    bill = table['Bill'].iloc[-1]
    #writer.writerow([bill/1.3,2,13.5,"MPC"])


    table = pd.read_csv('mpc_2_bill/{}_2_mpc_bill/sb8b64.csv'.format(i))
    bill = table['Bill'].iloc[-1]
    writer.writerow([bill/1.3,2,6.4,"MPC"])
    table = pd.read_csv('mpc_2_bill/{}_2_mpc_bill/sb8b135.csv'.format(i))
    bill = table['Bill'].iloc[-1]
    #writer.writerow([bill/1.3,2,13.5,"MPC"])

    table = pd.read_csv('op_2/{}_2_op/sb-8b64.csv'.format(i))
    bill = table['total_reward'].iloc[8735]-table['total_reward'].iloc[167]
    writer.writerow([bill/1.3,2,6.4,"Optimal"])
    table = pd.read_csv('op_2/{}_2_op/sb-8b135.csv'.format(i))
    bill = table['total_reward'].iloc[8735]-table['total_reward'].iloc[167]
...
bills = pd.read_csv("violin/violin_policy8.csv")
...
plt.savefig("violin/8-controller_added.png")
plt.show()
