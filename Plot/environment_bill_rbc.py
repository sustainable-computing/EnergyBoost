...
class EnergyEnvironment:
    ...
    def step(self, action):
        print("sell back price is",self.sell_back)

        print("index is",self.current_index)
        ...
        tg_bill = -total_sell_back * self.sell_back
        ...
        # else:
        #     self.current_payment += 1
        fg_bill = self.total_price - tg_bill
        reward=-self.total_price
        self.total_price = 0
        ...
        # use noise
        # return_state = np.array([np.random.normal(self.state[self.current_index][5],sigma*2), np.random.normal(self.state[self.current_index][6],sigma*4), self.state[self.current_index][0],self.state[self.current_index][9],self.state[self.current_index][7]])
        # #predict
        ...
        return total_need_grid, return_state, np.float(reward), total_need_grid,total_sell_back,fg_bill,tg_bill,len(self.state) == self.current_index + 1
