...
class EnergyEnvironment:
    ...
    def step(self, action):
        ...
        charge_from_grid = max(action - self.ac, 0)
        total_usable_power = max(self.ac - action, 0)
        total_sell_back = max(total_usable_power - self.homeuse, 0)
        total_need_grid = charge_from_grid + max(self.homeuse - total_usable_power, 0)
        reward = 0
        ...
        # use noise
        self.homeuse = np.random.normal(self.state[self.current_index][5],sigma*4)
        self.ac = np.random.normal(self.state[self.current_index][6],sigma*8)
        return_state = np.array([self.homeuse, self.ac, self.state[self.current_index][0],self.state[self.current_index][9],self.state[self.current_index][7]])
        # #predict
    ...
    def reset(self):
        ...
        #return_state = np.copy(self.state[self.current_index][5:7])
        self.homeuse = self.state[self.current_index][5]
        self.ac = self.state[self.current_index][6]
        return_state = np.array([self.homeuse, self.ac, self.state[self.current_index][0],self.state[self.current_index][9],self.state[self.current_index][7]])
