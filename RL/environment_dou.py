...
class EnergyEnvironment:
    ...
    def init_ground_truth(self):
        ...
        with open(self.datafile, 'r') as csvfile:
            ...
            for row in reader:
                ...
                self.truth.append(row_data)

                print("\rGround truth setup progress: %5.2f%%" % (row_number * 100 / row_count), end='')

        print("\rGround truth setup finished. Total %i lines data." % row_count)
    ...
    def init_prediction(self):
        ...
        if not os.path.exists('data_sc/predicted_hhdata_86_2.csv'):
            print("No datafile was found. Run generatepower.py first.")
            raise ValueError

        with open('data_sc/predicted_hhdata_86_2.csv', 'r') as csvfile:
            reader = csv.reader(csvfile, delimiter=',')
            row_count = sum(1 for _ in reader)
        ...
        with open('data_sc/predicted_hhdata_86_2.csv', 'r') as csvfile:
            ...
            for row in reader:
                ...
                print("\rPrediction setup progress: %5.2f%%" % (row_number * 100 / row_count), end='')

        print("\rPrediction setup finished. Total %i lines data." % row_count)



    def step(self, action):
        print("in this step index is",self.current_index)
    ...
    def step_truth(self, action):
        print("In this truth step the index is",self.current_index)
        ...
        self.truth[self.current_index][8] = min(max(self.truth[self.current_index - 1][8]*(1-self.eff_pleak) + deltaB - self.eff_cleak, 0), self.maximum_battery)

        #self.truth[self.current_index][8] = max(action + self.truth[self.current_index - 1][8], 0)


        # How much we charge the battery from grid
        charge_from_grid = max(action - self.truth[self.current_index - 1][6], 0)
        total_usable_power = max(self.truth[self.current_index - 1][6] - action, 0)
        total_sell_back = max(total_usable_power - self.truth[self.current_index - 1][5], 0)
        total_need_grid = charge_from_grid + max(self.truth[self.current_index - 1][5] - total_usable_power, 0)
        reward = 0
        ...
        if self.charge_mode == "Austin":
            ...
            self.total_price += 0.056101 * total_need_grid
            if self.truth[self.current_index - 1][0].month <= 9 or \
               self.truth[self.current_index - 1][0].month >= 6:
                if self.truth[self.current_index - 1][0].hour < 19 and \
                   self.truth[self.current_index - 1][0].hour >= 13:
                    self.total_price += 0.127314 * total_need_grid
        elif self.charge_mode == "TOU":
            #print("Using TOU price")
            if self.truth[self.current_index - 1][7]==1:
                #print("weekday")
                if self.truth[self.current_index - 1][9] < 11 and self.truth[self.current_index - 1][9] >= 5:
                    if (self.truth[self.current_index - 1][0] < 11 and self.truth[self.current_index - 1][0] >= 7) or \
                    (self.truth[self.current_index - 1][0] < 19 and self.truth[self.current_index - 1][0] >= 17):
                        self.total_price += 0.094 * total_need_grid
                    if self.truth[self.current_index - 1][0] < 17 and self.truth[self.current_index - 1][0] >= 11:
                        self.total_price += 0.132 * total_need_grid
                    if self.truth[self.current_index - 1][0] < 7 or self.truth[self.current_index - 1][0] >= 19:
                        self.total_price += 0.065 * total_need_grid
                if self.truth[self.current_index - 1][9] < 5 or self.truth[self.current_index - 1][9] >= 11:
                    if (self.truth[self.current_index - 1][0] < 11 and self.truth[self.current_index - 1][0] >= 7) or \
                    (self.truth[self.current_index - 1][0] < 19 and self.truth[self.current_index - 1][0] >= 17):
                        self.total_price += 0.132 * total_need_grid
                    if self.truth[self.current_index - 1][0] < 17 and self.truth[self.current_index - 1][0] >= 11:
            ...
            #print("using hour price")
            #self.total_price += self.hour_price[self.current_index - 1][0]][self.truth[self.current_index - 1][0].weekday()] * total_need_grid
            self.total_price += self.hour_price[self.current_index - 1] * total_need_grid
        # if self.current_payment == self.payment_cycle:
        #     reward = -self.total_price
        #     self.current_payment = 1
        ...
        reward=-self.total_price
        self.total_price = 0
        #return_truth = np.copy(self.truth[self.current_index][5:7])
        sigma=0.05
        # use real data
        return_state = np.array([self.truth[self.current_index][5],self.truth[self.current_index][6],self.truth[self.current_index][0],self.truth[self.current_index][9],self.truth[self.current_index][7]])
        return total_need_grid, return_state, np.float(reward), len(self.truth) == self.current_index + 1
    ...
    def reset_truth(self):
        ...
        self.total_price=0
        #print("in this reset process, battery starter is",self.battery_starter)
        self.truth[self.current_index][8] = self.truth[self.current_index-1][8]
        #return_truth = np.copy(self.truth[self.current_index][5:7])
        return_state = np.array([self.truth[self.current_index][5],self.truth[self.current_index][6],self.truth[self.current_index][0],self.truth[self.current_index][9],self.truth[self.current_index][7]])
        # return_truth[0] = float(return_truth[0].hour)
        # return_truth[9] = float(return_state[9].month)
        return np.hstack(return_state)
