...
def run(env, month_var, battery_var, agent, num_episodes, discount_factor=1.0):
    ...
    replay_buffer = utils.ReplayBuffer(BATCH_SIZE)
    total_steps = 0
    #state = env.reset()
...
#                action_learn = agent.select_action(state)
            ...
            if total_steps > BATCH_SIZE:
                agent.update_parameters(replay_buffer, BATCH_SIZE, 1)
            total_steps += 1
            ...
            replay_buffer.push(state, action, reward, featurize_state(next_state), mask)
...
if __name__ == '__main__':
    ...
    #plotting.plot_episode_stats(stats,homeid,sell_back_round,battery_round,name='SAC_'+str(homeid)+"_"+str(sell_back_round)+"_"+str(battery_round),smoothing_window=10)
